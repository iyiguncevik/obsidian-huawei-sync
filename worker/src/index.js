...
const OBSIDIAN_AUTH_URL = "obsidian://huawei-sync/auth";
  ...
  try {
    ...
    if (!result.ok || !result.body) {
      logEvent({
        ...
        huaweiError: result.error ?? null,
        huaweiSubError: result.subError ?? null,
      });
      return redirectToObsidian({ error: "token_exchange_failed", state });
    }

    const openid = result.body.openid ?? result.body.sub ?? "";
...
async function handleRefresh(request, env) {
  ...
  try {
    ...
    logEvent({ event: "refresh_success" });
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (pathname === "/client-id" && request.method === "GET") {
      if (!env.HUAWEI_CLIENT_ID) {
        logEvent({ event: "client_id_config_error" });
        return jsonResponse({ error: "server_config" }, 500);
      }
      return jsonResponse({ client_id: env.HUAWEI_CLIENT_ID });
    }
